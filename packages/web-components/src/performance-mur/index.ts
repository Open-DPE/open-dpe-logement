...
const THRESHOLDS: [number, number, number] = [0.3, 0.45, 0.65];
...
export class PerformanceMur extends BasePerformance {
	...
	protected parse(): { color: string; text: string } | null {
		...
		if (Number.isNaN(u)) {
			console.warn(`PerformanceMur: Invalid u value: ${u}`);
			return null;
		}
...
define("performance-mur", PerformanceMur);
