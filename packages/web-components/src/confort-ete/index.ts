...
import { CONFORT_ETE_COLORS } from "../shared/colors.js";

export function renderConfortEte(props: {
	value: models.diagnostic.ConfortEte;
}): string {
	const { value } = props;
	const color = CONFORT_ETE_COLORS[value] ?? "#000000";
	const style = `background-color: ${color}; color: #FFFFFF; padding: 4px;`;
...
const HTML_TAG = "open-dpe-logement-confort-ete";
