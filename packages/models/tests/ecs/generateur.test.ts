...
import {
  ...
  type ReseauChaleur,
  type ChaudiereCombustion,
} from '../../src/ecs/generateur.js'
import { UUID, p } from '../helpers.js'

const BASE_SIGNALETIQUE = {
  pn: null, cop: null, label: null, mode_combustion: null,
  presence_ventouse: null, pveilleuse: null, qp0: null, rpn: null,
}

describe('isGenerateur ECS — guard', () => {
  it('accepte un chauffe-eau électrique vertical', () => {
    const gen: ChauffeEauElectrique = {
      id: UUID,
    ...
    expect(isGenerateur(gen)).toBe(true)
  })

  it('accepte un chauffe-eau thermodynamique air ambiant', () => {
    const gen: ChauffeEauThermodynamique = {
      id: UUID,
  ...
  it('accepte un réseau de chaleur ECS', () => {
    const gen: ReseauChaleur = {
      ...
      stockage: null,
      signaletique: { pn: null, cop: null, label: null, mode_combustion: null, presence_ventouse: null, pveilleuse: null, qp0: null, rpn: null },
    }
    expect(isGenerateur(gen)).toBe(true)
  })

  it('accepte une chaudière à combustion ECS', () => {
    const gen: ChaudiereCombustion = {
      id: UUID,
