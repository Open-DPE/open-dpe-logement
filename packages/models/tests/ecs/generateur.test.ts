...
describe('Generateur ECS — types', () => {
  it('est une union de variantes', () => {
    expectTypeOf<Generateur>().toMatchTypeOf<ChauffeEauElectrique | ReseauChaleur | ChauffeEauThermodynamique>()
  })

  it('ChauffeEauElectrique a energie electricite', () => {
    expectTypeOf<ChauffeEauElectrique['energie']>().toEqualTypeOf<'electricite'>()
  })
...
describe('isGenerateur ECS — guard', () => {
  ...
  it('rejette si type est invalide', () => {
    expect(isGenerateur({ ...VALID_CHAUFFE_EAU_ELECTRIQUE as object, type: 'ballon_thermodynamique' })).toBe(false)
  })
