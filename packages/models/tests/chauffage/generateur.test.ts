...
import { isGenerateur, type Generateur, type PAC, type ReseauChaleur, type PoeleBouilleur } from '../../src/chauffage/generateur.js'
...
describe('Generateur chauffage — types', () => {
  it('est une union de variantes', () => {
    expectTypeOf<Generateur>().toMatchTypeOf<PAC | ReseauChaleur | PoeleBouilleur>()
  })
  ...
  it('PoeleBouilleur a energie bois', () => {
    expectTypeOf<PoeleBouilleur['energie']>().toEqualTypeOf<'bois_buche' | 'bois_plaquette' | 'bois_granule'>()
  })
})
...
describe('isGenerateur chauffage — guard', () => {
  ...
  it('rejette si type est invalide', () => {
    expect(isGenerateur({ ...VALID_PAC as object, type: 'cheminee' })).toBe(false)
  })
  ...
  it('rejette si energie est invalide', () => {
    expect(isGenerateur({ ...VALID_PAC as object, energie: 'vapeur' })).toBe(false)
  })
