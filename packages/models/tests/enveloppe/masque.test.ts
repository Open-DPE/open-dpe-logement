...
import { isMasque, type Masque, type MasqueLointainNonHomogene } from '../../src/enveloppe/masque.js'

// ─── Types ───────────────────────────────────────────────────────────────────

describe('Masque — types', () => {
  it('type est requis', () => {
    expectTypeOf<Masque['type']>().toMatchTypeOf<string>()
  })
  ...
  it('MasqueLointainNonHomogene a secteur obligatoire', () => {
    ...
    expectTypeOf<MasqueLointainNonHomogene['hauteur']>().toEqualTypeOf<number>()
  })
})
...
describe('isMasque — guard', () => {
  ...
  it('rejette si type est invalide', () => {
    expect(isMasque({ ...VALID_MASQUE_HOMOGENE as object, type: 'rideau_arbres' })).toBe(false)
  })
