...
import { isBatiment, type Batiment, type Maison, type Immeuble } from '../../src/batiment/batiment.js'
...
describe('Batiment — types', () => {
  it('type est une union Maison | Immeuble', () => {
    expectTypeOf<Batiment>().toMatchTypeOf<Maison | Immeuble>()
  })
  ...
  it('surface_habitable est un PositiveNumber (requis)', () => {
    expectTypeOf<Batiment['surface_habitable']>().toEqualTypeOf<PositiveNumber>()
  })
  ...
  it('Maison a appartements_visites tableau vide', () => {
    expectTypeOf<Maison['appartements_visites']>().toEqualTypeOf<[]>()
  })
})
...
describe('isBatiment — guard', () => {
  ...
  it('rejette si type est absent', () => {
    expect(isBatiment({ ...VALID_MAISON as object, type: undefined })).toBe(false)
  })

  it('rejette si type est invalide', () => {
    expect(isBatiment({ ...VALID_MAISON as object, type: 'villa' })).toBe(false)
  })

  it('rejette si surface_habitable est absent', () => {
    const { surface_habitable: _, ...rest } = VALID_MAISON as { surface_habitable: unknown }
