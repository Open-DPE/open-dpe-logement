# @open-dpe-logement/engine-abaques

Tables de valeurs forfaitaires utilisées par l'application de la méthode 3CL-DPE 2021.

Package privé (`private: true`), non publié sur npm — usage interne à `@open-dpe-logement/engine`, seul consommateur actuel.

## Installation

```sh
npm i @open-dpe-logement/engine-abaques
```
...
| Commande              | Effet                                        |
| --------------------- | -------------------------------------------- |
| `npm run generate`    | Compile les abaques `doctrine/` → `data/`    |
